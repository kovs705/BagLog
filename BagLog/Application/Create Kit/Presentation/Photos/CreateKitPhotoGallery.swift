...
struct CreateKitPhotoGallery: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: CreateKitDesign.cardSpacing) {
            if photos.isEmpty {
                PhotosPicker(
                    selection: $selection,
                    maxSelectionCount: CreateKitDraft.maximumPhotoCount,
                ...
                ScrollView(.horizontal) {
                    HStack(spacing: CreateKitDesign.cardSpacing) {
                        ...
                        if photos.count < CreateKitDraft.maximumPhotoCount {
                            PhotosPicker(
                                selection: $selection,
                ...
                Text(
                    "Photo 1 is the cover · \(photos.count) of \(CreateKitDraft.maximumPhotoCount)"
                )
                    .font(.subheadline)
                    .foregroundStyle(.white.opacity(0.68))
            }
    ...
    private var remainingSelectionCount: Int {
        max(0, CreateKitDraft.maximumPhotoCount - photos.count)
    }
